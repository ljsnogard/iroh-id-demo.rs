...
async fn main() -> Result<()> {
    ...
    let mode = args
        .next()
        .context("missing mode: use one of `server`, `client`, `local-server`, or `local-client`")?;
    ...
    match mode.as_str() {
        ...
        "local-server" => run_local_server().await,
        "local-client" => {
            let server_id = args
                .next()
                .context("missing server ID")?
            ...
            run_local_client(server_id).await
        }
        _ => {
            bail!(
                "unknown mode `{mode}`\n\
...
const LOCAL_PORT: u16 = 56789;

async fn run_local_server() -> Result<()> {
    use std::net::{SocketAddrV4, Ipv4Addr};
    // Bind strictly to localhost on an ephemeral port (0) or a specific port
...
async fn run_local_client(server_id: EndpointId) -> Result<()> {
    use std::net::{SocketAddrV4, Ipv4Addr};
    ...
    let local_addr = TransportAddr::Ip(SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::LOCALHOST, LOCAL_PORT)));
    let server_addr = EndpointAddr::from_parts(server_id, vec![local_addr]);
